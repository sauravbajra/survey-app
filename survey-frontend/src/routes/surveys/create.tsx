...
// This payload no longer needs survey_id
...
function CreateSurveyPage() {
  ...
  const surveyMutation = useMutation({
    ...
    onSuccess: async (data) => {
      ...
      try {
        ...
        // navigate({ to: '/surveys/$surveyId', params: { surveyId: newSurveyId } });
        navigate({ to: '/' });
      } catch (error: any) {
         toast({
  ...
  const handleAddQuestion = () => {
    setQuestions([...questions, { id: Date.now(), question_title: '', question_type: 'TEXT', options: [] }]);
  };
      ...
      <Box bg="white" p={8} borderRadius="lg" boxShadow="base">
        <VStack spacing={6} align="stretch">
          ...
          <Box p={4} borderWidth={1} borderRadius="md">
            ...
            <RadioGroup onChange={(v) => setPublishOption(v as 'immediately' | 'later')} value={publishOption}>
          ...
          {questions.map((q, index) => (
            <VStack key={q.id} p={4} borderWidth={1} borderRadius="md" align="stretch" spacing={4}>
              ...
              {q.question_type !== 'TEXT' && (<FormControl isRequired><FormLabel>Options (comma-separated)</FormLabel><Textarea placeholder="e.g., Good, Neutral, Bad" value={q.options.join(', ')} onChange={(e) => handleQuestionChange(index, 'options', e.target.value)} /></FormControl>)}
            </VStack>
          ))}
