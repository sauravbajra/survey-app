...
import { CheckCircle, Edit, Share2, Trash2 } from 'lucide-react';
...
function SurveyDetailPage() {
  ...
  const { data: surveyData, isLoading: isLoadingSurvey, isError: isErrorSurvey } = useQuery({
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
        toast({ title: 'Deletion failed.', description: error.response?.data?.message, status: 'error' });
    }
  });
  ...
  const handleShareClick = () => {
      ...
      const publicUrl = `${window.location.origin}/public/surveys/${surveyId}`;
  ...
  if (isErrorSurvey) {
    return (
        <Box p={4}>
            <Heading size="lg" color="gray.500">Survey Not Found</Heading>
      ...
      <Box>
        <Flex justify="space-between" align="center" mb={2} wrap="wrap" gap={4}>
          <Box>
            ...
            <Text color="gray.500" mt={1}>Status: {survey.status}</Text>
          </Box>
          <HStack spacing={2}>
            <Button
            ...
            <Button
                ...
                onClick={() => navigate({ to: '/surveys/$surveyId/edit', params: { surveyId } })}
                colorScheme="blue"
            >
                Edit
            </Button>
        ...
        <Tabs variant="enclosed-colored" colorScheme="blue" mt={6}>
          ...
          <TabPanels bg="white" borderRadius="lg" boxShadow="base" mt={-1}>
              <TabPanel>
                  <Heading size="md" mb={4}>Survey Questions</Heading>
                  <List spacing={3}>
                      {survey.questions.map((q: any) => (
                          <ListItem key={q.question_id}>
                              <ListIcon as={CheckCircle} color="green.500" />
                              {q.title} <Text as="span" color="gray.500">({q.type})</Text>
                          </ListItem>
                      ))}
                  </List>
              </TabPanel>
              <TabPanel>
                  {isLoadingSubmissions ? <LoadingSpinner /> : (
                      <Table variant="simple">
                          <Thead><Tr><Th>Submission ID</Th><Th>Submitted At</Th></Tr></Thead>
                          <Tbody>
              ...
              <TabPanel>
                  {isLoadingAnalytics ? <LoadingSpinner /> : (
                      <VStack align="stretch" spacing={6}>
                          ...
                          {analytics?.results.map((result: any) => (
                              <Box key={result.question_id} p={4} borderWidth={1} borderRadius="md">
                                  <Heading size="sm">{result.question_title}</Heading>
                                  <List mt={2} spacing={1}>
                                      {Object.entries(result.answer_frequencies).map(([answer, count]) => (
                                          <ListItem key={answer}>
                                              <Text as="span" fontWeight="bold">{answer}:</Text> {count as number} response(s)
                                          </ListItem>
                                      ))}
                                  </List>
                              </Box>
